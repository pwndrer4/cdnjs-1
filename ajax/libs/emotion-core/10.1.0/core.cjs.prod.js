...
var _inheritsLoose__default = _interopDefault(_inheritsLoose), css__default = _interopDefault(css), jsx = function(type, props) {
  ...
  var emotionProps = emotionElement.createEmotionProps(type, props), childrenLength = args.length - 2;
  if (1 === childrenLength) emotionProps.children = args[2]; else if (childrenLength > 1) {
    for (var childArray = new Array(childrenLength), i = 0; i < childrenLength; i++) childArray[i] = args[i + 2];
    emotionProps.children = childArray;
  }
  return React.createElement(emotionElement.Emotion, emotionProps);
