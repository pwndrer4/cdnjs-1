...
(function (global, factory) {
    ...
    var Component = {
        ...
        update: {
            ...
            write: function() {
                ...
                if (items.length < 2 || items.concat(this.target).some(uikitUtil.Transition.inProgress)) {
                    return;
                }

                var targetIndex = findTarget(items, x, y, uikitUtil.index(placeholder));
    ...
    function findTarget(items, x, y, current) {
        ...
        for (var i = 0; i < rects.length; i++) {
            var rect = rects[i];
            if (uikitUtil.pointInRect({x: x, y: y}, rect)) {
