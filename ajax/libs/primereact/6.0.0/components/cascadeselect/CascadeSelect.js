  ...
  _createClass(CascadeSelect, [{
    ...
    value: function onInputKeyDown(event) {
      switch (event.key) {
        ...
        case 'ArrowDown':
          if (this.state.overlayVisible) {
            _DomHandler.default.findSingle(this.overlay, '.p-cascadeselect-item').children[0].focus();
    ...
    value: function onOverlayEnter() {
      this.overlay.style.zIndex = String(_DomHandler.default.generateZIndex());
      this.alignOverlay();
    }
  }, {
    ...
    value: function alignOverlay() {
      ...
      if (this.appendTo) {
        _DomHandler.default.absolutePosition(this.overlay, container);

        this.overlay.style.minWidth = _DomHandler.default.getOuterWidth(container) + 'px';
      } else {
        _DomHandler.default.relativePosition(this.overlay, container);
      }
    }
  }, {
    ...
    value: function isOutsideClicked(event) {
      return this.container && !(this.container.isSameNode(event.target) || this.container.contains(event.target) || this.overlay && this.overlay.element && this.overlay.element.contains(event.target));
    }
  }, {
