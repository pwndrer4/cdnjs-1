...
(function (global, factory) {
  ...
  const FieldContext = Symbol('vee-validate-field-instance');
  ...
  function useField(name, rules, opts) {
      ...
      vue.provide(FieldContext, field);
  ...
  function useIsFieldDirty(path) {
      const form = injectWithSelf(FormSymbol);
      let field = path ? undefined : vue.inject(FieldContext);
      return vue.computed(() => {
          if (path) {
  ...
  function useFieldValue$1(path) {
      ...
      const field = path ? undefined : vue.inject(FieldContext);
      return vue.computed(() => {
          var _a;
          if (path) {
