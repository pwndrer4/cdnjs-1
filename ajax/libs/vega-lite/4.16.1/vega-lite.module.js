import { hasOwnProperty, isNumber, isString, writeConfig, splitAccessPath, stringValue, toSet, isObject, isBoolean as isBoolean$1, logger, Warn, isArray, isFunction, array, mergeConfig, identity } from 'vega-util';
import { __rest, __classPrivateFieldSet, __classPrivateFieldGet } from 'tslib';
...
function timeUnitSpecifierExpression(timeUnit) {
    ...
    return `timeUnitSpecifier(${stableStringify(timeUnitParts)}, ${stableStringify(VEGALITE_TIMEFORMAT)})`;
}
/**
...
const clear = {
    ...
    parse: (model, selCmpt, selDef) => {
        if (selDef.clear) {
            selCmpt.clear = isString(selDef.clear) ? selector(selDef.clear, 'scope') : selDef.clear;
        }
    },
    topLevelSignals: (model, selCmpt, signals) => {
...
const legendBindings = {
    ...
    parse: (model, selCmpt, selDef, origDef) => {
        ...
        const stream = isString(evt) ? selector(evt, 'view') : array(evt);
...
function parseUnitSelection(model, selDefs) {
    ...
    for (const name of keys(selDefs !== null && selDefs !== void 0 ? selDefs : {})) {
        ...
        const selCmpt = (selCmpts[safeName] = Object.assign(Object.assign({}, selDef), { name: safeName, events: isString(selDef.on) ? selector(selDef.on, 'scope') : duplicate(selDef.on) }));
        forEachTransform(selCmpt, txCompiler => {
