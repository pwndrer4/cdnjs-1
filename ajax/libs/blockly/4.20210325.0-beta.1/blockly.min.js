...
Blockly.Block.prototype.interpolate_=function(a,b,c,d){a=Blockly.utils.tokenizeInterpolation(a);this.validateTokens_(a,b.length);b=this.interpolateArguments_(a,b,c);c=[];a=0;for(var e;e=b[a];a++)switch(e.type){case "input_value":case "input_statement":case "input_dummy":if(e=this.inputFromJson_(e,d)){for(var f=0,g;g=c[f];f++)e.appendField(g[0],g[1]);c.length=0}break;default:(f=this.fieldFromJson_(e))&&c.push([f,e.name])}};
...
Blockly.Block.prototype.interpolateArguments_=function(a,b,c){for(var d=[],e=0;e<a.length;e++){var f=a[e];"number"==typeof f&&(f=b[f-1]);if("string"==typeof f&&(f=this.stringToFieldJson_(f),!f))continue;d.push(f)}a=d.length;b=Blockly.utils.string.startsWith;a&&b(d[a-1].type,"field_")&&(a={type:"input_dummy"},c&&(a.align=c),d.push(a));return d};
...
Blockly.Block.prototype.inputFromJson_=function(a,b){var c={LEFT:Blockly.ALIGN_LEFT,RIGHT:Blockly.ALIGN_RIGHT,CENTRE:Blockly.ALIGN_CENTRE,CENTER:Blockly.ALIGN_CENTRE},d=null;switch(a.type){case "input_value":d=this.appendValueInput(a.name);break;case "input_statement":d=this.appendStatementInput(a.name);break;case "input_dummy":d=this.appendDummyInput(a.name)}if(!d)return null;a.check&&d.setCheck(a.check);a.align&&(c=c[a.align.toUpperCase()],void 0===c?console.warn(b+"Illegal align value: ",a.align):
d.setAlign(c));return d};Blockly.Block.prototype.stringToFieldJson_=function(a){return(a=a.trim())?{type:"field_label",text:a}:null};Blockly.Block.prototype.appendInput_=function(a,b){var c=null;if(a==Blockly.INPUT_VALUE||a==Blockly.NEXT_STATEMENT)c=this.makeConnection_(a);a==Blockly.NEXT_STATEMENT&&this.statementInputCount++;a=new Blockly.Input(a,b,this,c);this.inputList.push(a);return a};
...
this.toolbox_=new (Blockly.registry.getClassFromOptions(Blockly.registry.Type.TOOLBOX,this.options))(this)}this.grid_&&this.grid_.update(this.scale);this.recordDeleteAreas();a=Blockly.registry.getClassFromOptions(Blockly.registry.Type.CURSOR,this.options);this.markerManager_.setCursor(new a);this.renderer_.createDom(this.svgGroup_,this.getTheme());return this.svgGroup_};
...
d)).connection){a=!0;break}}a&&(c=this.RTL?c-Blockly.SNAP_RADIUS:c+Blockly.SNAP_RADIUS,d+=2*Blockly.SNAP_RADIUS)}while(a);b.moveBy(c,d)}}finally{Blockly.Events.enable()}Blockly.Events.isEnabled()&&!b.isShadow()&&Blockly.Events.fire(new Blockly.Events.BlockCreate(b));b.select()};
...
a=(a=c.mutationToDom())&&Blockly.Xml.domToText(a);b=c.rendered;c.rendered=!1;c.compose(this.rootBlock_);c.rendered=b;c.initSvg();c.rendered&&c.render();b=(b=c.mutationToDom())&&Blockly.Xml.domToText(b);if(a!=b){Blockly.Events.fire(new Blockly.Events.BlockChange(c,"mutation",null,a,b));var g=Blockly.Events.getGroup();setTimeout(function(){Blockly.Events.setGroup(g);c.bumpNeighbours();Blockly.Events.setGroup(!1)},Blockly.BUMP_DELAY)}this.workspace_.isDragging()||this.resizeBubble_();Blockly.Events.setGroup(!1)}}};
