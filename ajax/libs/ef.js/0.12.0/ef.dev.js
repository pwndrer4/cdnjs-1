(function (global, factory) {
	 ...
	 */
	var getNamespace = function (perfix) {
	 ...
	 * @returns {void}
	 */
	var declareNamespace = function (perfix, namespaceURI) {
	...
	var getAttrHandler = function (element, key, custom) {
		...
		if (key.indexOf(':') > -1) {
			var ref = key.split(':');
			var perfix = ref[0];
			var namespace = getNamespace(perfix);
			return function (val) {
				// Remove attribute when value is empty
	...
	var addAttr = function (ref) {
		...
		var handler = getAttrHandler(element, key, custom);
	...
	var create = function (ref) {
		...
		var custom = Object.isPrototypeOf.call(shared.EFBaseComponent, ctx.scope[info.t] || info.t);

		var localNamespace = false;
