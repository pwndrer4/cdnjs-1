...
function setAriaRef(el, attr) {
  ...
  log$1('set aria ref', attr, value, innerRef);
...
const verifyRoot = asyncDebounce((name) => {
  log$1('root context verification');
...
function removeRulesById(id, namespace) {
  log$1('clean css rules by element id', id);
...
function transferCSS(id, root) {
  ...
  log$1('transfer styles to the shadow root:', JSON.stringify(id), root);
...
function incorrectTheme(prop, value) {
  log$1(`incorrect '${prop}' value in theme attribute`, value);
}
...
function declareTheme(el, name, hue, saturation, pastel, defaultMods) {
  log$1('declare theme', { element: el, name, hue, saturation, pastel, defaultMods });

  if (devMode && !el.nuContext) {
    log$1('element context not found');
    return;
  }
...
function applyTheme(element, { name, hue, saturation, pastel, type, contrast, lightness }, themeName) {
  ...
  log$1('apply theme', { element, themeName, hue, saturation, pastel, type, contrast, lightness });
...
function declareProp(Element, name) {
  log$1(`compatibility mode for [${name}] attribute`);
...
class Behavior {
  ...
  doAction(action, value) {
    ...
    if (action) {
      ...
      log$1('trigger action', this.host, action, value, actionCallback);
  ...
  emit(name, detail = null, options = {}) {
    if (name !== 'log') {
      log$1('emit', { element: this, name, detail, options });
    }
...
const BEHAVIORS = {
  focus: () => extractModule(import('./focus-b001c7f5.js')),
  hover: () => extractModule(import('./hover-30260c09.js')),
  listbox: () => extractModule(import('./listbox-92daa2b9.js')),
  option: () => extractModule(import('./option-49269450.js')),
  active: () => extractModule(import('./active-05e4dc4d.js')),
  fixate: () => extractModule(Promise.resolve().then(function () { return fixate; })),
  orient: () => extractModule(import('./orient-5ef56191.js')),
  popup: () => extractModule(import('./popup-b6b22aad.js')),
  control: () => extractModule(import('./control-7b20b0c8.js')),
  radiogroup: () => extractModule(import('./radiogroup-ced30752.js')),
  action: () => extractModule(import('./action-f2efd48d.js')),
  label: () => extractModule(import('./label-8eba1e31.js')),
  code: () => extractModule(import('./code-5c240cd8.js')),
  markdown: () => extractModule(import('./markdown-7374c5bc.js')),
  datetime: () => extractModule(import('./datetime-231d3800.js')),
  number: () => extractModule(import('./number-58e1f582.js')),
  slider: () => extractModule(import('./slider-5507f2e2.js')),
  numinput: () => extractModule(import('./numinput-4d779a34.js')),
  input: () => extractModule(import('./input-84b54cfa.js')),
  textarea: () => extractModule(import('./textarea-bf31ab7e.js')),
  fileinput: () => extractModule(import('./fileinput-fe4d751c.js')),
  icon: () => extractModule(import('./icon-24d3fb8d.js')),
  svg: () => extractModule(import('./svg-cbb102c8.js')),
  image: () => extractModule(import('./image-6938d32f.js')),
  debug: () => extractModule(import('./debug-5acbea49.js')),
  debugger: () => extractModule(import('./debugger-9a97b043.js')),
  form: () => extractModule(import('./form-69f5d89b.js')),
  validator: () => extractModule(import('./validator-9c75470d.js')),
  group: () => Promise.resolve(GroupBehavior),
  tooltip: () => extractModule(import('./tooltip-94423058.js')),
  progressbar: () => extractModule(import('./progressbar-e61a1269.js')),
  value: () => extractModule(import('./value-61c37ca5.js')),
  datepicker: () => extractModule(import('./datepicker-8b61f1e8.js')),
  dateinput: () => extractModule(import('./dateinput-754a4ef0.js')),
  inputgroup: () => extractModule(import('./inputgroup-c9d606e4.js')),
  menu: () => extractModule(import('./menu-e7fca021.js')),
  menuitem: () => extractModule(import('./menuitem-85cee7b9.js')),
  offset: () => extractModule(import('./offset-d00efffc.js')),
  appear: () => extractModule(import('./appear-90d91025.js')),
  hotkey: () => extractModule(import('./hotkey-0723b3f5.js')),
  current: () => extractModule(import('./current-7ebe30dc.js')),
};
...
class NuAbstract extends HTMLElement {
  ...
  static nuGenerateDefaultStyle(isHost = false, dontInject = false) {
    ...
    log$1('default style generated', tag);
  ...
  disconnectedCallback() {
    ...
    if (this.nuDisconnectedHooks) {
      ...
      log$1('disconnected hooks', { el: this });
    }
  ...
  nuVerifyChildren(options) {
    ...
    log$1('verify children', { vars, responsive, attrs, shadow, selector });
...
function assignOption(element, prop, value, elements = {}, replace) {
  ...
  log$1('property assigned:', `${element.nuTag}.${prop}:`, newValue);
}
...
class NuProps extends NuDefinition {
  ...
  nuApply() {
    ...
    Object.entries(props).forEach(([varName, varValue]) => {
      ...
      log$1('apply property', { context: parent, name: varName, value: value });
    });
  }
}
...
class NuBase extends NuDefinition {
  ...
  nuApply() {
    ...
    log$1('set base:', { value: normalizedValue });
  }
}
...
var generators = {
  set(name, cb) {
    ...
    log$1('new style generator is defined');
  },
  define(name, cb) {
