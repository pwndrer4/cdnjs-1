/*!
  ...
  * @license MIT
  */
import { ref, unref, toRef, getCurrentInstance, onBeforeUnmount } from 'vue-demi';
...
const rtdbPlugin = function rtdbPlugin(app, pluginOptions = defaultOptions) {
    ...
    app.config.globalProperties[unbindName] = function rtdbUnbind(key, reset) {
        internalUnbind(key, rtdbUnbinds.get(this), reset);
        delete this.$firebaseRefs[key];
    };
    // add $rtdbBind and $rtdbUnbind methods
    app.config.globalProperties[bindName] = function rtdbBind(key, source, userOptions) {
...
const firestorePlugin = function firestorePlugin(app, pluginOptions = defaultOptions$1) {
    ...
    const { bindName, unbindName } = globalOptions;
    app.config.globalProperties[unbindName] = function firestoreUnbind(key, reset) {
        internalUnbind$1(key, firestoreUnbinds.get(this), reset);
        delete this.$firestoreRefs[key];
    };
    app.config.globalProperties[bindName] = function firestoreBind(key, docOrCollectionRef, userOptions) {
        const options = Object.assign({}, globalOptions, userOptions);
        const target = toRef(this.$data, key);
...
export { bind$1 as firestoreBind, firestorePlugin, unbind$1 as firestoreUnbind, bind as rtdbBind, rtdbPlugin, unbind as rtdbUnbind };
