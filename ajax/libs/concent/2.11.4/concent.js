  ...
  function changeRefState(state, _temp, targetRef) {
    ...
    function (renderType, committedState, updateRef) {
      ...
      callMiddlewares(skipMiddleware, passToMiddleware, function () {
        ...
        var needExtract = force === false; // 如果设置了 force 为true，表示不比较变化，即needExtract为false

        var realShare = saveSharedState$1(module, passToMiddleware.sharedState, needExtract); // TODO: 查看其它模块的cu函数里读取了当前模块的state或computed作为输入产生了的新的计算结果
