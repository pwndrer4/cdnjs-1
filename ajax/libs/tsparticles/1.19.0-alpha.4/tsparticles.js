...
;// CONCATENATED MODULE: ./dist/browser/Core/Particle/Velocity.js
class Velocity {
  ...
  get length() {
    return Math.sqrt(Math.pow(this.horizontal, 2) + Math.pow(this.vertical, 2));
  }
  ...
  getLengthSq() {
    return Math.pow(this.horizontal, 2) + Math.pow(this.vertical, 2);
  }
  ...
  rotate(angle) {
    return new Velocity(this.horizontal * Math.cos(angle) - this.vertical * Math.sin(angle), this.horizontal * Math.sin(angle) + this.vertical * Math.cos(angle));
  }
...
function circleBounceDataFromParticle(p) {
  return {
    ...
    radius: p.getRadius(),
    velocity: p.velocity,
    factor: new Velocity(getRangeValue(p.options.bounce.horizontal.value), getRangeValue(p.options.bounce.vertical.value))
  };
}
...
function rectBounce(particle, divBounds) {
  ...
  }, particle.velocity.horizontal, getRangeValue(particle.options.bounce.horizontal.value));
  ...
  const resV = rectSideBounce({
    ...
    min: divBounds.left,
    max: divBounds.right
  }, particle.velocity.vertical, getRangeValue(particle.options.bounce.vertical.value));
...
class Particle {
  constructor(id, container, position, overrideOptions, group) {
    ...
    if (this.options.move.spin.enable) {
      ...
      this.spin = {
        center: spinCenter,
        direction: this.velocity.horizontal >= 0 ? "clockwise" : "counter-clockwise",
  ...
  moveParticle(particle, delta) {
    ...
    if (particlesOptions.move.spin.enable) {
      ...
      if (particlesOptions.move.vibrate) {
        this.moveXY(particle, Math.sin(particle.position.x * Math.cos(particle.position.y) * zVelocityFactor), Math.cos(particle.position.y * Math.sin(particle.position.x) * zVelocityFactor));
      }
    }
...
class AbsorberInstance {
  ...
  updateParticlePosition(particle, angle, acceleration) {
    ...
    if (this.options.orbits) {
      ...
      particle.absorberOrbitAngle += particle.moveSpeed * container.retina.pixelRatio / 100 * container.retina.reduceFactor;
    } else {
...
class EmittersPlugin {
  ...
  needsPlugin(options) {
    ...
    if (emitters instanceof Array) {
      ...
      loadEmitters = true;
    } else if (((_c = (_b = (_a = options.interactivity) === null || _a === void 0 ? void 0 : _a.events) === null || _b === void 0 ? void 0 : _b.onClick) === null || _c === void 0 ? void 0 : _c.mode) && isInArray("emitter", options.interactivity.events.onClick.mode)) {
      loadEmitters = true;
    }
...
class Lighter extends ExternalBase {
  ...
  isEnabled() {
    ...
    const hoverMode = events.onHover.mode;
    return isInArray("light", hoverMode);
  }
